...
especially the transitions and copy in the createTextureImage function. Try to experiment with this by creating a setupCommandBuffer that the helper functions
record commands into, and add a flushSetupCommands to execute the commands that have been recorded so far.
...
private:
	...
	void drawFrame()
	{
		...
		// According to https://vulkan-tutorial.com/Drawing_a_triangle/Drawing/Rendering_and_presentation , this is needed to prevent memory leaks when validation layers are
		// enabled.
	...
	void initVulkan()
	{
		...
		// Start recording a command buffer that records all staging and copy operations for vertex/index/texture data
		beginSetupCommandBuffer();

		// Depth resources only need to be transitioned, no staging/copying is necessary
		createDepthResources();

		createFramebuffers();

		// Setup stagingBuffer and stagingBufferMemory for every call, as we need to clean that up after the setup buffer was submitted.
		VkBuffer stBufTex, stBufVert, stBufIdx;
		VkDeviceMemory stBufMemTex, stBufMemVert, stBufMemIdx;

		createTextureImage(stBufTex, stBufMemTex);
		createVertexBuffer(stBufVert, stBufMemVert);
		createIndexBuffer(stBufIdx, stBufMemIdx);

		// Submit all commands that were setup
		submitSetupCommandBuffer();

		// Cleanup staging buffers
		vkDestroyBuffer(device, stBufTex, nullptr);
		vkDestroyBuffer(device, stBufVert, nullptr);
		vkDestroyBuffer(device, stBufIdx, nullptr);
		vkFreeMemory(device, stBufMemTex, nullptr);
		vkFreeMemory(device, stBufMemVert, nullptr);
		vkFreeMemory(device, stBufMemIdx, nullptr);
	...
	void createTextureImageView() { textureImageView = createImageView(textureImage, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_ASPECT_COLOR_BIT); }

	void copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height)
	{
		VkCommandBuffer commandBuffer = setupCommandBuffer; // beginSingleTimeCommands();
	...
	void createTextureImage(VkBuffer& stagingBuffer, VkDeviceMemory& stagingBufferMemory)
	{
		...
		copyDataToCPUVisibleBuffer(pixels, imageSize, stagingBuffer, stagingBufferMemory);
	...
	void createIndexBuffer(VkBuffer& stagingBuffer, VkDeviceMemory& stagingBufferMemory)
	{
	...
	void createVertexBuffer(VkBuffer& stagingBuffer, VkDeviceMemory& stagingBufferMemory)
	{
	...
	void copyDataToCPUVisibleBuffer(const std::vector<T>& src_data, VkBuffer buffer, VkDeviceMemory memory)
	{
		...
		size_t buffer_size = sizeof(T) * src_data.size();
	...
	template <typename T>
	void copyDataToCPUVisibleBuffer(T* src_data, size_t src_data_size, VkBuffer buffer, VkDeviceMemory memory)
	{
		// TODO: do we need this?
		// vkDeviceWaitIdle(device);

		size_t buffer_size = sizeof(T) * src_data_size;
	...
	void createRenderPass()
	{
		...
		// This determines what is done with the data in the attachment before and after rendering
	...
	void createSwapChain()
	{
		...
		// Swap chain surface format, presenation mode and extent
		...
		{
			// Different graphics queue family and presentation queue family
	...
	{
		auto queue_family_indices = findQueueFamilies(physicalDevice);

		std::vector<VkDeviceQueueCreateInfo> queueCreateInfos;
		std::set<int> uniqueQueueFamilies = {queue_family_indices.graphicsFamily, queue_family_indices.presentFamily};
		...
		vkGetDeviceQueue(device, queue_family_indices.graphicsFamily, 0, &graphicsQueue);
		vkGetDeviceQueue(device, queue_family_indices.presentFamily, 0, &presentQueue);
	}

	VkSurfaceFormatKHR chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats)
	{
		// When only a single entry with format VK_FORMAT_UNDEFINED is given, the surface has no preferred format
		if(availableFormats.size() == 1 && availableFormats[0].format == VK_FORMAT_UNDEFINED)
		{
			return {VK_FORMAT_B8G8R8A8_UNORM, VK_COLOR_SPACE_SRGB_NONLINEAR_KHR};
		}

		// If we're not free to choose, we'll go through the list and try to find the preferred combination
	...
	int rateDeviceSuitability(VkPhysicalDevice device)
	{
		...
		// We need to have adequate swap chain support
